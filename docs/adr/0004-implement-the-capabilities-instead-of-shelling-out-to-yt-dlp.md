...
- **Transcripts are the shared weak point.** `youtube_transcript` does not go through the Data API; it reads the caption track the way yt-dlp does, so it inherits the same breakage class when YouTube changes the player or the timedtext endpoint. Search, details, and subscriptions use the documented API and do not.
